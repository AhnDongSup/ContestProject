...
api_key = 'AIzaSyB4kIuWFdytMzw5nYjzmYpzkqYl5KNbRs0'
...
for result in range(len(df)):
    ...
    video_title.append(request['items'][0]['snippet']['title']) # 제목 추출
    ...
    for result in comment_response['items']:
        ...
        img_comments.append([comment['authorDisplayName'], comment['textDisplay'], comment['likeCount']]) # 댓글 작성자, 내용, 좋아요 수 추출
...
final_df.to_json(f'E:/GitHub/IFP/project/ContestProject_OneSearch/ContestProject_OneSearch/{today}.json', orient = 'columns', force_ascii=False)
