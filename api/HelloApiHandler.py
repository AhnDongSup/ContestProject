...
class HelloApiHandler(Resource):
    def post(self):
        name = request.get_json()['name']   # get the JSON that comes back from our AJAX request.
        conn = sqlite3.connect('words.db')
        cur = conn.cursor()
        cur.execute(f"INSERT INTO word VALUES ('{name}')")
        conn.commit()
        return 'POST ok'
    ...
    def get(self):
        ...
        #     result = request.form
        # words.append('#쇼츠' + result)
        # words.append('#쇼츠 ')
        # 'AIzaSyB4kIuWFdytMzw5nYjzmYpzkqYl5KNbRs0'
        ...
        result = final_df.to_json(orient="columns")
